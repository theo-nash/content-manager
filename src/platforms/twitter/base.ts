...
export interface _Tweet extends Tweet {
    language?: string;
    quotes?: number;
}
...
export class ClientBase extends EventEmitter {
            ...
            if (someCachedTweetsExist) {
                ...
                for (const tweet of tweetsToSave) {
                    ...
                    await this.runtime.messageManager.createMemory({
                        ...
                        roomId,
                        embedding: content.text.length > 0 ? await embed(this.runtime, content.text) : getEmbeddingZeroVector(),
    ...
    async saveRequestMessage(message: Memory, state: State, additionalEmbed?: string) {
        if (message.content.text) {
            ...
            if (
                ...
                await this.runtime.messageManager.createMemory({
                    ...message,
                    embedding: message.content.text.length > 0 ? await embed(this.runtime, message.content.text.toString() + (message.content?.imageDescription || "") + (additionalEmbed || "")) : getEmbeddingZeroVector(),
                });
            }
